...
def parse_slopd_report(verbose=False, save_raw=False):
    try:
        ...
        with urllib.request.urlopen(
            "https://pdreport.slocity.org/policelog/rpcdsum.txt"
        ) as f:
        ...
        if save_raw:
            with open(
                f"data/raw_files/SLOPD_report_{today}.txt", "w", newline="\n"
            ) as f:
                f.write(text)
    ...
    try:
        ...
        for chunk_num, chunk_text in enumerate(text.split(SEPARATOR)):
            ...
            if chunk_num > 0:
                ...
                if chunk_num % 2 == 1:
                    ...
                    if verbose:
                        print(
                            f'Processing header for report number {report_counter} \n{"-"*40}\n{chunk_text}'
                        )

                    #                             210819016 08/19/21         Received:08:11 Dispatched:      Arrived:08:12 Cleared:08:12
                    header_pattern = re.compile(
                        "(\d{9}) (\d{2}/\d{2}/\d{2}) Received:(.*?) Dispatched:(.*?) Arrived:(.*?) Cleared:(.*?)"
                    )
                ...
                if chunk_num % 2 == 0:
                    ...
                    for _num, _text in enumerate(chunk_text.split("\n")):
                        ...
                        if type_pattern_matches:
                            extracted_cases[report_counter][
                                "type"
                            ] = type_pattern_matches.group(1).strip()
                            extracted_cases[report_counter][
                                "grid_location"
                            ] = type_pattern_matches.group(2).strip()
                        ...
                        if location_pattern_matches:
                            ...
                            extracted_cases[report_counter][
                                "address"
                            ] = location_pattern_matches.group(1).strip()
                            extracted_cases[report_counter][
                                "location"
                            ] = location_pattern_matches.group(2).strip()
                            extracted_cases[report_counter][
                                "clearance_code"
                            ] = location_pattern_matches.group(3).strip()
                            extracted_cases[report_counter][
                                "observed_crime"
                            ] = observed_crime.strip()
                        ...
                        if observed_crime_flag:
                            observed_crime = observed_crime.replace(
                                "As Observed:", ""
                            ) + _text.strip().replace("As Observed:", "")

                        resp_officer_pattern = re.compile("Responsible Officer: (.*)")
                        resp_officer_pattern_matches = resp_officer_pattern.search(
                            _text
                        )
                        if resp_officer_pattern_matches:
                            extracted_cases[report_counter][
                                "responsible_officer"
                            ] = resp_officer_pattern_matches.group(1).strip()
                        ...
                        if units_pattern_matches:
                            extracted_cases[report_counter][
                                "units"
                            ] = units_pattern_matches.group(1).replace(" ", "")
                        ...
                        if des_pattern_matches:
                            extracted_cases[report_counter][
                                "description"
                            ] = des_pattern_matches.group(1).strip()
                        ...
                        if cc_pattern_matches:
                            extracted_cases[report_counter][
                                "call_comments"
                            ] = cc_pattern_matches.group(1).strip()
        ...
        for key, value in extracted_cases.items():
            ...
            for key_report, value_report in value.items():
                if verbose:
                    print(
                        f'     {key_report}: {"-"*(30 - len(key_report))}-> {value_report}'
                    )
