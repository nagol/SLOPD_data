...
def get_address_geoinfo(police_data, user_agent="SLOPD_parser", verbose=False):
    ...
    addresses.rename(
        columns={0: "street_address", 1: "address_description"}, inplace=True
    )
    addresses.street_address = addresses.street_address.str.split("#").str[
        0
    ]  # remove unit numbers from address, ie 215 Chorro #15
    ...
    for i, address in enumerate(addresses.street_address):
        ...
        try:
            ...
            if verbose:
                print(
                    "No results returned. Either address not found or input address malformed or ambiguous."
                )
...
location_dict = get_address_geoinfo(
    police_data=pd_data.head(), user_agent="SLOPD_parser", verbose=True
)
